...
//! method, `Toggle`. Client side: `ghostwriter --toggle` calls it and exits.
...
pub const PATH: &str = "/io/github/hmrdsmoke/GhostWriter";
...
#[zbus::interface(name = "io.github.hmrdsmoke.GhostWriter")]
impl Server {
        ...
        cosmic::iced::stream::channel(4, |tx: Sender<Message>| async move {
            ...
            match builder {
                Ok(builder) => match builder.build().await {
                    Ok(_connection) => {
                        eprintln!("ghostwriter: hotkey endpoint up on D-Bus as {NAME}");
                        // Keep the connection alive forever.
                        futures::future::pending::<()>().await;
                    }
                    Err(e) => eprintln!(
                        "ghostwriter: could not own D-Bus name {NAME} ({e}); hotkey disabled"
                    ),
                },
                Err(e) => eprintln!("ghostwriter: D-Bus setup failed ({e}); hotkey disabled"),
            }
...
/// Asks the running applet to toggle. Used by `ghostwriter --toggle`.
